...
function Speech(texts, options) {
  ...
  function pickEngine() {
    ...
    if (isSupertonicVoice(options.voice)) {
      console.info('[BookNote][TTS ROUTE] Speech.pickEngine -> SupertonicTtsEngine', {voiceName:options.voice.voiceName, modelId:options.voice.localModelId, voiceId:options.voice.localVoiceId, lang:options.lang});
  ...
  .subscribe({
    next(event) {
      ...
      if (typeof self.onEvent === "function") {
        try { self.onEvent(event) } catch (e) { console.error("[BookNote][TTS EVENT]", e) }
      }
      switch (event.type) {
        case "start":
    ...
    return playbackState$.pipe(
      ...
      rxjs.scan((playing$, state) => {
        if (state == "resumed") {
          ...
          if (playing$) {
            if (isAndroid() || isChromeOSNative(options.voice)) {
              engine.stop()
              return null
            } else {
              engine.pause()
