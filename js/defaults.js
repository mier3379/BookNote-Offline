...
const voices$ = rxjs.combineLatest({
  ...
  rxjs.switchMap(settings => Promise.all([
    ...
    (settings.supertonicVoices || []).filter(v => {
      if (!v) return false;
      const n = String(v.voiceName || v.name || "");
      return !v.isSupertonicLocal && !v.localModelId && !/^Supertonic Local /.test(n) && !/^Supertonic-ZH /.test(n);
    }),
  ])),
...
function isSupertonicVoice(voice) {
  return /^Supertonic(?:-ZH)? /.test(voice.voiceName)
}
...
async function getSpeechVoice(voiceName, lang) {
  const allVoices = await rxjs.firstValueFrom(voices$)

  // Neural voices are not browser-local voices, so they must be resolved before
  // the historical isOfflineVoice filter. Reader TTS uses this same resolver.
  if (voiceName && (isPiperVoice({voiceName:String(voiceName)}) || isSupertonicVoice({voiceName:String(voiceName)}))) {
    const neural = allVoices.find(v => String(v && (v.voiceName || v.name) || "") === String(voiceName));
    if (neural) return neural;

    // A locally registered voice can be persisted outside the filtered registry.
    // Keep exact-name compatibility for those registrations when available.
    const stored = await getSetting(isPiperVoice({voiceName:String(voiceName)}) ? "piperVoices" : "supertonicVoices");
    const local = Array.isArray(stored) ? stored.find(v => String(v && (v.voiceName || v.name) || "") === String(voiceName)) : null;
    if (local) return local;
  }

  // Original resolution for browser-local/system voices.
  const voices = allVoices.filter(isOfflineVoice)
  let voice;
  ...
  if (!voice && lang && preferredVoiceByLang) {
    const preferredName = preferredVoiceByLang[parseLocaleCode(lang).language];
    if (preferredName) voice = findVoiceByName(voices, preferredName);
  }

  if (!voice && lang) {
    voice = voices.find(voice =>
      voice.lang && parseLocaleCode(voice.lang).language == parseLocaleCode(lang).language
    )
  }
  return voice;
}
