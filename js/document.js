...
function SimpleSource(texts, opts) {
  ...
  this.ttsOptions = opts.ttsOptions || null
  this.ttsEventHandler = opts.ttsEventHandler || null
...
function TabSource(tabId) {
    ...
    // VitalSource/Chegg ---------------------------------------------------------
    {
      ...
      getTexts: function(tab) {
        ...
        return tryGetFrame(5000)
          .then(function(frame) {
            if (frame) return getFrameTexts(tab.id, frame.frameId, ["js/jquery-3.7.1.min.js", "js/messaging.js", "js/content/vitalsource-book.js"]);
            else return null;
          })
      },
  ...
  function injectScripts() {
    return inject("js/jquery-3.7.1.min.js")
    ...
    function read(texts) {
      ...
      return Promise.resolve()
        ...
        .then(function(speech) {
          if (source.ttsEventHandler) speech.onEvent = source.ttsEventHandler;
  ...
  function getSpeech(texts) {
    return getSettings()
      .then(function(settings) {
        var overrides = source.ttsOptions || {};
        console.log("Declared", info.lang)
        console.log("Detected", info.detectedLang)
        var lang = overrides.lang || ((!info.detectedLang || info.lang && info.lang.startsWith(info.detectedLang)) ? info.lang : info.detectedLang);
        console.log("Chosen", lang)
        var options = {
          rate: Number(overrides.rate) || settings.rate || defaults.rate,
          pitch: Number(overrides.pitch) || settings.pitch || defaults.pitch,
          volume: Number(overrides.volume) || settings.volume || defaults.volume,
          lang: config.langMap[lang] || lang || 'en-US',
        }
        var voicePromise = overrides.voice
          ? Promise.resolve(overrides.voice)
          : getSpeechVoice(overrides.voiceName || settings.voiceName, options.lang);
        return voicePromise.then(function(voice) {
